...
func (p Provider) loginHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := p.JwtService.Set(w, &claims, false); err != nil {
		SendErrorJSON(w, r, http.StatusInternalServerError, err, "failed to set jwt")
		return
	}
...
func (p Provider) authHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		SendErrorJSON(w, r, http.StatusInternalServerError, err, "failed to get jwt")
		return
	}
	...
	if err != nil {
		SendErrorJSON(w, r, http.StatusInternalServerError, err, "exchange failed")
		return
	}
	...
	if err != nil {
		SendErrorJSON(w, r, http.StatusBadRequest, err, fmt.Sprintf("failed to get client info via %s", p.InfoURL))
		return
	}
	...
	if err != nil {
		SendErrorJSON(w, r, http.StatusInternalServerError, err, "failed to read user info")
		return
	}

	jData := map[string]interface{}{}
	if e := json.Unmarshal(data, &jData); e != nil {
		SendErrorJSON(w, r, http.StatusInternalServerError, err, "failed to unmarshal user info")
		return
	}
	...
	if err = p.JwtService.Set(w, claims, oauthClaims.SessionOnly); err != nil {
		SendErrorJSON(w, r, http.StatusInternalServerError, err, "failed to save user info")
		return
	}
...
func (p Provider) setPermissions(u User, siteID string) User {
